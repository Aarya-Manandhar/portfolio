    ...
    <section id="notes" className="py-16 md:py-24 border-t border-flat max-w-6xl mx-auto px-4 md:px-8">
      ...
      <div className="space-y-4">
        {TECHNICAL_NOTES.map((note) => (
          <div
            ...
            className="group p-6 rounded-2xl bg-surface border-flat space-y-3 cursor-pointer hover:border-accent transition-all duration-200"
          >
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {note.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 rounded-md bg-surface-subtle border-flat text-[10px] font-medium text-muted"
                >
                  {tag}
                </span>
              ))}
      ...
      {selectedNote && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="w-full max-w-3xl bg-surface border-flat rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
            {/* Modal Header */}
            <div className="p-6 border-b border-flat bg-surface-subtle flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="p-1.5 rounded-lg border-flat text-muted hover:text-primary hover:bg-surface transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 border-t border-flat bg-surface-subtle flex items-center justify-between">
              <div className="flex gap-1.5">
                {selectedNote.tags.map((t) => (
                  <span key={t} className="px-2 py-0.5 rounded-md bg-surface border-flat text-[10px] text-muted">
                    {t}
                  </span>
                ))}
              </div>
